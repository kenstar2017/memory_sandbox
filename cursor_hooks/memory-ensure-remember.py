...
marker 由 memory-mark.py 在 memory_remember / memory_update / memory_delete
成功后写下，这里检查完就删掉：marker 表示「距上次 stop 之间落过库」，所以多轮
对话里每一轮都会独立判断。改掉旧条目也算落库——否则「更新而不是新增」这种
正确做法会被追问，逼出一条重复记忆。

读侧标记（.prepared / .gated / .pack）也在这里按轮清掉，这样「每轮都要先查记忆」，
而不是一个会话里查一次就永久放行。正常情况下 beforeSubmitPrompt 在每轮开头就清过一次，
这里是兜底：预取 hook 没装、或那一轮没走提问入口（如 followup）时仍要清干净。
"""
...
def _nested_run() -> bool:
    """
    这一轮是记忆沙箱自己拉起来的嵌套 agent 吗（`MEMORY_SANDBOX_NESTED=1`）。

    评论机器人 / IM 机器人把本机 Cursor agent CLI 当模型用，那个进程带
    `--approve-mcps`，于是它也有记忆沙箱 MCP、也继承这套 hook。让它落库的结果是
    **一次问答落两条**：嵌套 agent 自己写一条，机器人再写一条。嵌套 agent 只负责
    产出一段文本，记忆归调用方统一写，所以这里直接放过。
    """
...
FOLLOWUP = (
    "【记忆沙箱·落库兜底】本轮既没有写入新记忆，也没有更新已有记忆。"
    "若本轮产生了可复用结论，现在必须落进长时记忆：\n"
    ...
    "本轮结论若是在修正已有记忆（取值变了、规范改了、结论被推翻），"
    "用它的 id 调 memory_update 改掉、整条失效就 memory_delete，"
    "这同样算落库——别为了消掉这条提示再写一条新的，新旧并存会让后续检索打架。\n"
...
def _reset_turn_state(conv: str) -> None:
    """清掉本轮的读侧标记与没投递出去的预取包，下一轮必须重新查记忆。"""
